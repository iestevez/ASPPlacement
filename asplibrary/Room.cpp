...
void Room::moveobjects(std::vector<std::unique_ptr<Worldobject>>& olist) {
	...
	for (std::vector<std::unique_ptr<Worldobject>>::iterator it = Room::VObjects.begin(); it != Room::VObjects.end(); it++)
		olist.push_back(std::move(*it));
...
if(clingo_signature_is_equal_to(sig_placement,symbolsignature)){
	...
	for(auto& obj :((Room *)object)->VObjects){
		if(obj->getid()==objid){
			int newx;
			newx = ((Room *)object)->getxfromwallrelative(obj, wall, objpos, objrot);
			std::cout << "Pos x: " << newx;
			int newy;
			newy = ((Room *)object)->getyfromwallrelative(obj, wall, objpos, objrot);
			std::cout << " Pos y: " << newy << std::endl;
			obj->setplacement(newx,newy,objrot,wall,objpos);		
			break;
		}
	}
...
switch(w){
	...
	case Wall::Name::c:
		if (rot == Worldobject::Rotation::a || rot == Worldobject::Rotation::c)
			ret = (VWalls.at(0)->get_length()) - pos - (obj->getdimx());
		else
			ret = (VWalls.at(0)->get_length()) - pos - (obj->getdimy());
		break;
	case Wall::Name::d:
		ret = 0;
		break;
...
switch(w){
	...
	case Wall::Name::d:
		if (rot == Worldobject::Rotation::a || rot == Worldobject::Rotation::c)
			ret = (VWalls.at(1)->get_length()) - pos - (obj->getdimy());
		else
			ret = (VWalls.at(1)->get_length()) - pos - (obj->getdimx());
		break;
...
int  Room::placeObjectAt(int id, int x,int y, Worldobject::Rotation rot, Wall::Name wall, int wr){

int ret=0;
for(auto& obj : VObjects){

if(obj->getid() == id){
	obj->setplacement(x,y,rot,wall,wr);
	ret=1;
	break;
